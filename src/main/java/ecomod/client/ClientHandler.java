...
public class ClientHandler
{
	...
	{
		if(smog_intensity.compareTo(0) > 0  && !Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.blindness))
		{
			Vec3 color = EMUtils.lerp(Vec3.createVectorHelper(event.red, event.green, event.blue), Vec3.createVectorHelper(66F/255, 80F/255, 67F/255), Math.min(smog_intensity.floatValue(), Minecraft.getMinecraft().theWorld.isDaytime() ? 0.9F : 0.55F));
	...
	{
		if(smog_intensity.compareTo(0) > 0 && event.fogMode != GL11.GL_EXP2 && event.fogMode != GL11.GL_EXP)
		{
			if(!Minecraft.getMinecraft().thePlayer.isPotionActive(Potion.blindness))
			{
				float f = event.farPlaneDistance;
		...
		if((event.entity != null && event.entity.ticksExisted % 2 == 0) && !Minecraft.getMinecraft().isGamePaused())
		{
