...
public class EcomodASMHooks
{
    ...
    {
        if(entity_item == null || entity_item.getEntityWorld().isRemote || dmg_source == null || entity_item.getEntityItem().stackSize < 1)
            return;
    ...
    public static void itemFoodOnEatenAddition(ItemStack stack, World worldIn, EntityPlayer player)
    {
        if(!worldIn.isRemote)
        {
            if(stack.hasCapability(EcomodStuff.CAPABILITY_POLLUTION, null))
            {
                ...
                if(m >= 60)
                {
                    player.addChatMessage(new TextComponentTranslation("msg.ecomod.polluted_food").setStyle(new Style().setColor(TextFormatting.DARK_GREEN)));
