...
public class TileAnalyzer extends TileEnergy implements ITickable
{
	...
	public Pair<Long, PollutionData> analyze()
	{
		...
		if(energy.getEnergyStored() == energy.getMaxEnergyStored())
		{
			...
			{
				worldObj.playSound(null, getPos(), EcomodStuff.analyzer, SoundCategory.BLOCKS, 3F, 1F);
			...
			{
				for(EntityPlayerMP pl : worldObj.getEntitiesWithinAABB(EntityPlayerMP.class, new AxisAlignedBB(getPos().add(-4, -4, -4), getPos().add(4, 4, 4))))
				{
					pl.addChatMessage(new TextComponentString("Analyzer is unable to work without a surface access!"));
				}
			}
		}
