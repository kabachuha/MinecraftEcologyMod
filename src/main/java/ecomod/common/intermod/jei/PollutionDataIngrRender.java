...
public class PollutionDataIngrRender implements IIngredientRenderer<PollutionData>
{
	...
	public void render(Minecraft minecraft, int xPosition, int yPosition, PollutionData pollution)
	{
		...
		if(!Minecraft.getMinecraft().getLanguageManager().getCurrentLanguage().getLanguageCode().toLowerCase().equals("en_us"))
		{
			try
			{
	...
	@Override
	public List<String> getTooltip(Minecraft minecraft, PollutionData ingredient) 
	{
	...
	@Override
	public FontRenderer getFontRenderer(Minecraft minecraft, PollutionData ingredient) {
